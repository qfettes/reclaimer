// Modified version of wrk.c to capture traces at milliseconds level using ualarm and using CURL command with the help of timeStamps in the interval. 
// The timestamps used will be that of the first and last trace which get generated in the interval
...
///////////////////////////////////////////////////////////////////////////////////
// Modified below to capture InProgressTraces every 10 seconds(Edited by Shaik Adil)

CURL *curl;
FILE *fp;
int res; // variable to store status of curl command

long long int elapsedTime = 0;  // variable to capture the elapsedTime since start of workload

char outfilename[FILENAME_MAX] = ""; // File to capture the traces information in the specific interval(10 seconds)
char traceNameStart[200] = "trace";
int fileNum =1;				 // create file to capture duration and traces in that duration(10 seconds)
char traceNameEnd[50] = ".json";

long long int Trace_time1 = 0; // variable to store timestamp of firstTrace after start of 10 second interval
long long int Trace_time2 = 0; // variable to store timestamp of lastTrace before end of 10 second interval

int TraceTracker = 1; // variable to keep track of Traces count


long long int startTime_msec; // Timestamp which records start of Workload Generator
bool flag = true; // boolean flag used for easier trace tracking

void sig_handler(int signum){

  flag = true;
  //alarm(10); // commented as ualarm will take care of scheduling
  sprintf(outfilename, "%s%d%s", traceNameStart,fileNum,traceNameEnd); 
  //printf("%s\n",outfilename);
  
  fp = fopen(outfilename,"w+"); 
  curl = curl_easy_init();

  char urlStart[200] = "http://localhost:16686/api/traces?service=compose-post-service&start=";
  char urlEnd[10] = "&end=";

  char url[FILENAME_MAX];

  sprintf(url, "%s%lld%s%lld",urlStart,Trace_time1,urlEnd,Trace_time2);
  ...
  TraceTracker = 1;
  if(res==0)
   printf("Succesfully captured file %d\n",fileNum);
  else
   printf("Failed to capture traces\n\n\n");
  fileNum++;
}
...
int main(int argc, char **argv) {
   ...
   // Added below code to capture startTime of Workload Generator(Edited by Shaik Adil)
  struct timeb timer_msec;
  if (!ftime(&timer_msec)) {
    startTime_msec = ((long long int) timer_msec.time) * 1000ll + 
  ...
  printf(" WorkLoad Generator StartTime -> %lld \n", startTime_msec);
  
  // Added below code to schedule the alarm every 10 seconds (Edited by Shaik Adil)
  signal(SIGALRM,sig_handler); // Register signal handler
  ualarm(500000,500000);  // Schedule the first alarm after 500 milliseconds and repeat every 500 milliseconds
...
//////////////////////////////////////////////////////////////////////////////////////////////////
// Modified below code to capture timeStamps of traces which were generated in the 10 second interval
...
static int response_complete(http_parser *parser) {
    ...
    thread->complete++; 
    //Start and End of traces
    
////////////////////////////////////////////////////////////////////////////////////////////////////
    // Calculating the timeElapsed and keeping track of Traces .(Edited by Shaik Adil)
    printf("Trace%d Start-> %"PRIu64"\n", TraceTracker,c->start);
    elapsedTime =  (now/1000)-startTime_msec; 
    printf("Total TimeElapsed-> %lld milliseconds\n\n", elapsedTime);
    TraceTracker++;

    // Using flag to keep track of startingTrace and EndingTrace every second.(Edited by Shaik Adil)
    if(flag == true)
    {
    flag = false;
    Trace_time1 = c->start;
...
static void print_hdr_latency(struct hdr_histogram* histogram, const char* description) {
    ...
    printf("\n%s\n", "  Detailed Percentile spectrum -adil:");
    hdr_percentiles_print(histogram, stdout, 5, 1000.0, CLASSIC);
}

static void print_stats_latency(stats *stats) {
    long double percentiles[] = { 50.0, 75.0, 90.0, 99.0, 99.9, 99.99, 99.999, 100.0 };
    printf("  Latency Distribution adil\n");
    for (size_t i = 0; i < sizeof(percentiles) / sizeof(long double); i++) {
        long double p = percentiles[i];
