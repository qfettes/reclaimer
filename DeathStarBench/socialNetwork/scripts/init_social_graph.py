...
def init_social_graph():
  filename_default = "../DeathStarBench/socialNetwork/datasets/social-graph/socfb-Reed98/socfb-Reed98.mtx"
  assert(os.path.isfile(filename_default))
...
#   random.seed(1)   # deterministic random numbers
  ...
  addr = "http://{}:{}".format(ip_default, port_default)
  loop = asyncio.get_event_loop()
  future = asyncio.ensure_future(register(addr, nodes))
  register_result = loop.run_until_complete(future)
  future = asyncio.ensure_future(follow(addr, edges))
  follow_result = loop.run_until_complete(future)
  
  return_val = True
  print(register_result)
  print(follow_result)
  for r in register_result + follow_result:
