...
def load_config(base_dir, old_config=None, whitelist=None):
    new_config = pickle.load(open(os.path.join(base_dir, 'config.dump'), 'rb'))
    new_config.load_model = True

    if (not (old_config is None)) and (not (whitelist is None)):
...
def create_directory(path, clear_existing=True):
    try:
        os.makedirs(path)
    except OSError:
        if clear_existing:
